...
data = pd.read_csv(
    'data/SCDB_2019_01_caseCentered_Citation.csv',
    encoding='ISO 8859-1')
...
# Selecting all float columns, they should have been int cols, due to Null
# values they are designated as Float columns
...
# updatining caseOrigin with court dict values, other values will be
# replaced with 9999
data['caseOrigin'] = data['caseOrigin'].apply(
    lambda x: x if x in court_dict.keys() else 9999)
# updatining caseSource with court dict values, other values will be
# replaced with 9999
data['caseSource'] = data['caseSource'].apply(
    lambda x: x if x in court_dict.keys() else 9999)
# updatining petitioner with top 15 values, other values will be replaced
# with 9999
petition_category = data['petitioner'].value_counts(ascending=False)[
    :15].index.values
data['petitioner'] = data['petitioner'].apply(
    lambda x: x if x in petition_category else 9999)
# updatining respondent with top 15 values, other values will be replaced
# with 9999
respondent_category = data['respondent'].value_counts(ascending=False)[
    :15].index.values
data['respondent'] = data['respondent'].apply(
    lambda x: x if x in petition_category else 9999)
...
layout = html.Div([
    ...
    html.H3('Viz Lab for Supreme Court Dataset',
        ...
        dbc.Row([
            ...
            dbc.Col(
                html.Div([
                    ...
                    dcc.Dropdown(
                        id='group_variable',
                        options=[{'label': background_variables_dict[key], 'value': key}
                                 for key in background_variables_dict],
                        value='petitioner',
                        clearable=False
                    )
                ]),
...
def update_groupvariable_values(groupvariable):
    ...
    group_variablevalues = [{'label': 'Select All',
                             'value': 'all'}] + [{'label': name,
                                                  'value': name} for name in data[groupvariable].unique()]
...
def update_drill_variable(selected_groupvalue, group_variable):
    if(selected_groupvalue != 'all'):
        drill_variable = [{'label': background_variables_dict[key], 'value': key}
                          for key in background_variables_dict if key != group_variable]
...
def update_drillvariable_values(variablename, drill_variable_disable):
    if(drill_variable_disable):
        ...
        drill_variable_name = 'Select ' + \
            background_variables_dict[variablename] + ':'
        drill_variablevalues = [{'label': 'Select All',
                                 'value': 'all'}] + [{'label': name,
                                                      'value': name} for name in data[variablename].unique()]
        return False, drill_variable_name, drill_variablevalues, 'all'

# Callback for updating the Supreme Courts Terms as per the User choice
# selected
...
def update_timevariable(variablename):
    ...
    if(variablename == 'All'):
        ...
        timevariablevalues = [{'label': str(decade) + '-' + str(
            decade % 100 + 9), 'value': decade} for decade in range(maxdecade, mindecade, -10)]
        time_defaultvalue = [maxdecade]
    else:
        timevariable_name = 'Select Year:'
        timevariablevalues = [{'label': year, 'value': year}
                              for year in range(maxyear, minyear, -1)]
...
# Instead of bar chart, a pie chart will be created if a specific
# groupvalue and drill value is selected
@app.callback(
...
def update_graph(
        group_variable,
        group_variablevalues,
        drill_variable,
        drill_variablevalues,
        timevariable,
        timevariablevalues):
    ...
    # For create a data which consists of cases belonging to the time feature
    # selected
    if(timevariable == 'All'):
        year_values = all_years
        ...
        for decade in timevariablevalues:
            # Avoiding dash bug , where previous radio object value is sent for
            # first time
            if(decade != 'All'):
                year_values = [
                    term for term in all_years if (
                        (term >= decade) & (
                            term <= (
                                decade + 9)))]
    else:
    ...
    column_df = year_data.groupby([group_variable])[
        'partyWinning'].count().reset_index()
    # removing the values which are less than 1% of dataset
    column_df = column_df[column_df['partyWinning'] > onepercent]
    column_pie = go.Pie(
        values=column_df['partyWinning'],
        hole=0.55,
        labels=column_df[group_variable],
        name='Break by Categories',
        showlegend=False,
    ...
    pie_chart = {
        'data': [column_pie],
        'layout': {
            'title': f'% of Cases by {background_variables_dict[group_variable]} , Time: ' + str(timevariable),
            'titlefont': {
                'size': 18,
                'color': '#287D95',
                'family': 'Raleway'}}}
    ...
    if((group_variablevalues == 'all')):
        ...
        result_data = year_data[(year_data[group_variable] == group_variablevalues)][[
            drill_variable, 'partyWinning']]
        result_variable = drill_variable
        titlevariable = group_variable
        variablevalues = group_variablevalues
    else:

        if(drill_variablevalues is not None):

            create_pie = True
            result_data = year_data[(year_data[group_variable] == group_variablevalues) & (
                year_data[drill_variable] == drill_variablevalues)]['partyWinning'].value_counts().reset_index()

    if(result_variable is not None):

        losing_df = result_data[result_data['partyWinning'] == 'Lost'].groupby(
            result_variable)['partyWinning'].count().reset_index()
        winning_df = result_data[result_data['partyWinning'] == 'Won'].groupby(
            result_variable)['partyWinning'].count().reset_index()
        winning_bar = go.Bar(
            x=winning_df['partyWinning'],
            y=winning_df[result_variable],
            name='Won',
            orientation='h')
        losing_bar = go.Bar(
            x=losing_df['partyWinning'],
            y=winning_df[result_variable],
            name='Lost',
            orientation='h')
        bar_graph = {
            'data': [
                ...
                losing_bar],
            'layout': {
                'title': f'Win/Loss Cases: {second_title} for [ {background_variables_dict[titlevariable]} : {variablevalues} ]',
                'colorway': [
                    "#287D95",
                    "#EF533B"],
                'textposition': 'inside',
        ...
        if(create_pie):
            bar_graph = {
                'data': [
                    go.Pie(
                        values=result_data['partyWinning'],
                        labels=[
                            'Lost',
                            'Won'],
                        hoverinfo='value')],
